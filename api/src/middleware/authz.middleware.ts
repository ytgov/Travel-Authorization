...
export async function ensureUserFromAuth0Token(token: string): Promise<User | null> {
  ...
  const auth0Subject = response.data?.sub

  if (isNil(auth0Subject)) {
    throw new Auth0PayloadError(response.data)
  }
