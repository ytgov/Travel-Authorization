      ...
      <v-form ref="form" lazy-validation>
        <v-row>
          <v-col cols="12" md="2">
            ...
            <v-checkbox v-model="currentForm.allTravelWithinTerritory" label="In Territory?" dense required>
            </v-checkbox>
          </v-col>
          <v-col cols="12" md="3">
            <v-select :value="tripType" :items="tripTypes" :rules="[required]" label="Trip Type" background-color="white"
              dense outlined required @change="updateTripType"></v-select>
          </v-col>
        </v-row>
        ...
        <v-row>
          <v-col cols="12" md="1">
            <v-text-field v-model="currentForm.travelDuration" :rules="[required, isNumber]" label="# Days"
              background-color="white" dense outlined required></v-text-field>
          </v-col>
          <v-col cols="12" md="2">
            <v-text-field v-model="currentForm.daysOffTravelStatus" :rules="[isNumber]" label="Days on non-travel status"
              background-color="white" dense required outlined></v-text-field>
          </v-col>
          <v-col cols="12" md="3">
            <DatePicker v-model="currentForm.dateBackToWork" :min="finalDestination.departureDate" :rules="[required]"
              text="Expected Date return to work" required />
          </v-col>
        </v-row>
      </v-form>
    </v-card-text>
