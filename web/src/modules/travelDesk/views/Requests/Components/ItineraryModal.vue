<template>
	<div>
		<v-dialog v-model="itineraryDialog" persistent  max-width="50%">
			...
			<v-card :loading="loadingData">
				<v-card-title color="red" class="primary white--text" >
					<div class="text-h5">
						Travel Itinerary
					</div>
					...
					<title-card class="mt-10" titleWidth="11rem" largeTitle >
						...
						<template #body>
							<div :id="'pdf-page-' + invoiceNumber">
								<div v-for="flightSegment,inx in flightSegments" :key="'flight-'+inx" class="one-section" style="margin-top:0.25rem;">																
									<div v-if="flightSegment.tkHeader" style="margin:1rem 0 1rem 0.75rem;">																				
										<div style="line-height:1.25rem;"><b>Ticket Number:</b> {{flightSegment.ticketNumber}}</div>
										<div style="line-height:1.25rem;" ><b>Passenger Name:</b> {{flightSegment.passengerName}}</div>											
										<div style="line-height:1.25rem;"><b>Total Cost:</b> $ {{totalCost}} </div>                            
									</div>  
									<table style="width:99%;" class="mx-2 mb-7">
										<tbody >											
											<tr style="line-height:1rem;">
												<td class="text-left" style="width:12%;" ><b>Flight:</b></td>
												<td colspan="2">{{flightSegment.flightNumber}}</td>                                
											</tr>                           
											<tr style="background:#F9F9F9;">
												<td class="text-left" style="width:12%;" ><b>Departure:</b></td>
												<td style="width:21%;" >{{flightSegment.departDate | beautifyDateTime}}</td>
												<td style="width:66%;" >{{flightSegment.departLocation}}</td>
											</tr>
											<tr style="line-height:1rem;">
												<td class="text-left" style="width:12%;" ><b>Arrival:</b></td>
												<td style="width:21%;" >{{flightSegment.arriveDate | beautifyDateTime}}</td>
												<td style="width:66%;" >{{flightSegment.arriveLocation}}</td>
											</tr>
											<tr style="background:#F9F9F9;">
												<td class="text-left" style="widtd:12%;" ><b>Class</b></td>
								...
								<div style="margin:1rem 0.25rem;white-space: pre;"><p>{{invoiceRemarks}}</p></div>
							</div>				
						</template>
					</title-card>	
				</v-card-text>
