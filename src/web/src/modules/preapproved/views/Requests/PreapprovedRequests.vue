<template>
    <div class="mx-10 mb-5"> 
        <v-row class="my-0 mx-0">
            <submit-travel v-if="admin" :disabled="selectedRequests.length==0" :travelRequests="travelRequests" :selectedRequests="selectedRequests" :preTSubID="0" @updateTable="updateTable" buttonName="Submit Selected Travel" class="ml-auto" />
            <print-report v-if="admin"  :disabled="selectedRequests.length==0" :travelRequests="selectedRequests" buttonName="Print Report" />  
        ...
        <template v-slot:item.edit={item}>
            <new-travel-request :type="item.status=='Draft' || !item.status?'Edit':'View'" @updateTable="updateTable" :travelRequest="item" />            
        </template>
    ...
    data () {
        return {
            headers: [                
                ...
                { text: '',             value: 'edit',       class: 'blue-grey lighten-4', cellClass:'px-0 mx-0', sortable:false, width: "1rem" },
            ],
            admin: false,
            selectedRequests: [],
            firstSelectionDept: ''
