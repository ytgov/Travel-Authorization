                    ...
                    >
                        ...
                        <template v-slot:item.status={item}>                                                       
                            <v-tooltip top color="amber accent-4">
                                <template v-slot:activator="{ on }">                                    
                                    <v-icon style="cursor:pointer" v-on="on" v-if="item.status && item.preTSubID!=preTSubID" class="" color="amber accent-2">mdi-alert</v-icon> 
                                </template>
                                <span class="black--text">
                                    This request is already in another submission.<br/> If you Save/Submit this change, it will be removed from the other submission.
                                </span>
                            </v-tooltip>                           
                        </template>
            ...
            <v-card>
                ...
                <v-card-text>
                    ...
                    <template v-slot:item.travelDate={item}>
                        ...
                        <div v-else><div>{{item.startDate|beautify-date}} to</div><div>{{item.endDate|beautify-date}}</div></div>            
                    </template>
...
export default {
    ...
    data () {
        return {
            headers: [                
                ...
                { text: '',sortable:false,value: 'status',   class: 'blue-grey lighten-4', cellClass:'px-0 mx-0', width: "1rem" },
                { text: '',sortable:false,value: 'remove',   class: 'blue-grey lighten-4', cellClass:'px-0 mx-0', width: "1rem" },
                { text: '',sortable:false,value: 'edit',     class: 'blue-grey lighten-4', cellClass:'px-0 mx-0', width: "1rem" },
            ],
            addTravelHeaders: [                
                { text: 'Name',         value: 'name',        class: 'blue-grey lighten-4'},
                ...
                { text: 'Location',     value: 'location',    class: 'blue-grey lighten-4' },
                { text: 'Purpose Type', value: 'purpose',     class: 'blue-grey lighten-4' },
                // { text: 'Reason',       value: 'reason',      class: 'blue-grey lighten-4' },
                { text: 'Status',       value: 'status',      class: 'blue-grey lighten-4' ,cellClass:'text-h6 red--text'},
            ],     
            submittingRequests: [],
    ...
    methods: { 
        ...
        extractTravelRequests(){
            this.submittingRequests = JSON.parse(JSON.stringify(
                this.selectedRequests.filter(req => req.status==null || req.status=='Draft')
            ))
        },

        removeTravel(item){
            this.submittingRequests = JSON.parse(JSON.stringify(this.submittingRequests.filter(travel => travel.preTID != item.preTID)))
            this.update++;
        },
        ...
        submitTravelRequest(type){
            ...
            if(currentIDs.length>0){
                ...
                // console.log(body)
                axios.post(`${PREAPPROVED_URL}/submissions/${this.preTSubID}`, body).then(() => {                                    
        ...
        deleteSubmission(){
            axios.delete(`${PREAPPROVED_URL}/submissions/${this.preTSubID}`).then(() => {                                    
                    this.savingData = false;
                    this.submitTravelDialog = false;
                    this.$emit('updateTable');
            }).catch(e => {this.savingData = false; console.log(e)});
