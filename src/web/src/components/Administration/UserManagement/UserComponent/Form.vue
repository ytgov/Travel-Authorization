...
export default {
    ...
    methods: {
        async saveUser() {    
            ...
            axios.put(`${USERS_URL}/${this.$route.params.id}/permissions`, permsObject).then((resp) => {
                console.log(resp);
        ...
        async loadDepartments() {
            return axios.get(`${LOOKUP_URL}/department-branch`).then((resp) => {
                for(const key of Object.keys(resp.data))
                    this.departments.push({name:key})
